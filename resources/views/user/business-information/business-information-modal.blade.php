    ...
    <div class="relative w-full max-w-7xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 flex flex-col gap-y-3">
                @if ($information)
                <form action="{{ route('user.business.update', ['information' => $information->id]) }}" method="POST">
                @else
                <form action="{{ route('user.business.store') }}" method="POST">
                @endif
                    @include('user.business-information.business-information-form')
